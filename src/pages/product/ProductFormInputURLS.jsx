...
export default function ProductForm2() {
  ...
  const staticData = {
    ...
    discount: 0,
    color: [{ color: "Fresh Green", images: ["https://images.unsplash.com/photo-1604977042946-1eecc30f269e?w=500"] }],
  ...
  const onSubmit = async (data) => {
    try {
      await createProduct({ ...data, ...staticData }).unwrap();
  ...
  const inputClass = (error) =>
    `w-full border rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition ${
    ...
    <>
      <div className="max-w-md mx-auto my-10 p-8 bg-white rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          <input {...register("name")} placeholder="Name" className={inputClass(errors.name)} />
          ...
          />
          {errors.stockQuantity && <p className="text-red-500 mt-1 text-sm">{errors.stockQuantity.message}</p>}

          <input
            {...register("priceIn", { valueAsNumber: true })}
            type="number"
            placeholder="Price In"
            className={inputClass(errors.priceIn)}
          />
          {errors.priceIn && <p className="text-red-500 mt-1 text-sm">{errors.priceIn.message}</p>}

          <input
            {...register("priceOut", { valueAsNumber: true })}
            type="number"
            placeholder="Price Out"
            className={inputClass(errors.priceOut)}
          />
          {errors.priceOut && <p className="text-red-500 mt-1 text-sm">{errors.priceOut.message}</p>}
